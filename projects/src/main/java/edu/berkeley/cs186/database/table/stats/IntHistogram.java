...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int min = getMinValue();
    int max = getMaxValue();
    int distinct = getNumDistinct();
    int range = max - min;
//    Float hey = new Integer(3);
/*
    switch (predicate) {
      case EQUALS:
        if (predValue >= min && predValue <= max) {
          return 1.0f/ distinct;
        } else {
          return 0;
        }
      ...
      case NOT_EQUALS:
        if (predValue >= min && predValue <= max) {
          return ((distinct - 1.0f)/ distinct);
        } else {
          return 0;
        }

      case LESS_THAN:
        float returnable = (predValue - min)/ (range);
        if (returnable > 1) {
          return 1;
...
//        if (predValue >= min && predValue <= max) {
//          return (predValue - min)/ (range);
//        } else {
//          return 0;
//        }

      case LESS_THAN_EQUALS:
        returnable = (predValue - min +1.0f) / ((range));
        if (returnable > 1) {
          return 1;
        } else if (returnable < 0 ) {
          return 0;
...
//        if (predValue >= min && predValue <= max) {
//          return (predValue - min +1.0f) / ((range));
//        } else {
//          return 0;
//        }

      case GREATER_THAN:
        returnable = (max - predValue) / ((range));
        if (returnable > 1) {
          return 1;
        } else if (returnable < 0 ) {
          return 0;
...
//        if (predValue >= min && predValue <= max) {
//          return (max - predValue) / ((range));
//        } else {
//          return 0;
//        }

      case GREATER_THAN_EQUALS:
        returnable = (max - predValue + 1.0f)/ (range);
//        if (predValue >= min && predValue <= max) {
//          return (max - predValue + 1.0f)/ (range);
//        } else {
//          return 0;
//        }
        if (returnable > 1) {
    ...
    */
    switch (predicate) {
      ...
      case NOT_EQUALS:
        return ((distinct - 1.0f)/ distinct);
      case LESS_THAN:
        return (float) (predValue - min)/ (range);
      case LESS_THAN_EQUALS:
        return (predValue - min +1.0f) / ((range));
      case GREATER_THAN:
        return (float) (max - predValue) / ((range));
      case GREATER_THAN_EQUALS:
        return (max - predValue + 1.0f)/ (range);
      default:
        break;
